...
function startFlashcardSession(){
    ...
    if(document.getElementById('flashDueMode')&&document.getElementById('flashDueMode').value!=='free')pool=pool.filter(function(f){return !mem[f.id]||Number(mem[f.id].due||0)<=Date.now();});
...
function renderFlashcard(){
    ...
    var f=flashcardDeck[flashcardIndex];if(!f){root.innerHTML='<div class="empty-state">'+(flashcardDeck.length?'Série terminée : '+flashcardDeck.length+' cartes revues.':'Aucune carte à réviser pour ces filtres. Tu peux choisir Entraînement libre.')+'</div>';return;}
