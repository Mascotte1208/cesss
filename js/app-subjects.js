...
function openChapterBplus(id) {
    ...
    var courseHtml = coursSections.length ? coursSections.map(function (section, index) {
        if (subject === 'histoire' || subject === 'bio' || subject === 'chimie') {
        ...
        return '<section class="cours-section bplus-accordion' + (open ? ' open' : '') + '" data-index="' + index + '">' +
            ...
            '<span class="bplus-section-number" aria-hidden="true">' + section.icon + '</span><span><small>' + escapeHtml(String(subjectInfo.label || subject).toUpperCase()) + '</small>' + escapeHtml(section.title) + '</span><span class="cours-chevron">' + (open ? '▾' : '▸') + '</span></button>' +
