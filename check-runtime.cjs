...
const nodes=new Map();const element=id=>{if(!nodes.has(id))nodes.set(id,{id,innerHTML:'',textContent:'',value:'',style:{},classList:{add(){},remove(){},contains(){return false}},querySelector(){return null},querySelectorAll(){return []},prepend(){},appendChild(child){this.innerHTML+=(child&&child.innerHTML)||'';},scrollIntoView(){},addEventListener(){},setAttribute(){},getAttribute(){return null;},removeAttribute(){},insertAdjacentHTML(position,html){if(position==='afterbegin')this.innerHTML=html+this.innerHTML;else this.innerHTML+=html;}});return nodes.get(id);};
...
const questions=library.flatMap(ch=>ch.exercices);assert.equal(questions.length,1287);
...
c.flashcardSubject='francais';element('flashYear').value='6e';element('flashChapter').value='all';element('flashCount').value='10';c.startFlashcardSession();assert.equal(c.flashcardDeck.length,10);const card=c.flashcardDeck[0];c.flipFlashcard();c.rateFlashcard(false);assert(c.cessState.flashLearning[card.id]);assert.equal(c.flashcardIndex,1);c.rateFlashcard(true);assert.equal(c.flashcardIndex,1);
c.cessSave();const saved=JSON.parse(storage.get('carnetCESSv4'));assert(saved.profile&&saved.mastery&&saved.legacyMistakes);
console.log('PASS: all scripts, 14 subjects, 279 fiches / 952 QCM + 335 open questions, profile migration, navigation, search results, memo folders, flashcard content, answer mapping, progress, simulation and duplicate completion.');
